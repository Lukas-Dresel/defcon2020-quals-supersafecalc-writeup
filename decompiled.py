...
# Decompiled from: Python 3.7.7 (default, Apr 18 2020, 02:59:53) 
# [GCC 9.3.0]
# Embedded file name: ./supersafecalc.py
# Compiled at: 2020-05-14 23:53:56
...
def main(pfname):
    ...
    for v, ttype in t:
        ...
        if ttype == 'op':
            ...
            code += bytes.fromhex('48b9') + struct.pack('<Q', FUNCTIONS + findex * 256)
    ...
    #   -   
    #
    # PTRACE_O_EXITKILL - kill child after parent dies
    #   -   apparently for jails so the child can never run without the parent
    #
    # PTRACE_O_TRACECLONE
    #   -   stop tracee at clone, trace new process
    #
    # PTRACE_O_TRACE_VFORK
    #   -   stop tracee at vfork, trace new process
    #
    #
    #
    #
    # Notes:
    #   -   missing PTRACE_O_TRACE_FORK
    #
    ptrace(PTRACE_SETOPTIONS, pid, 0, PTRACE_O_TRACEEXIT | PTRACE_O_TRACESECCOMP | PTRACE_O_EXITKILL | PTRACE_O_TRACECLONE | PTRACE_O_TRACEVFORK)
    ...
    while True:
        ...
        print(pid, parsed_status)
        if parsed_status[1] == 'WIFEXITED':
            print("EXIT NORMALLY")
            break
        if parsed_status[2] == 'SIGSEGV':
            res = ptrace(PTRACE_GETREGS, pid, 0, ctypes.addressof(regs))
            print("EXIT COZ SIGSEGV!!! RIP")
            print("RIP:{} RSP: {} RAX: {} ORIG_RAX: {} RDI: {} RSI: {}".format(*list(map(hex, [regs.rip, regs.rsp, regs.rax, regs.orig_rax, regs.rdi, regs.rsi]))))
            break
        elif parsed_status[2] == 'SIGTRAP' and parsed_status[3] in {'0', }:#'PTRACE_EVENT_EXIT'}:
            res = ptrace(PTRACE_GETREGS, pid, 0, ctypes.addressof(regs))
            print("RIP:{} RSP: {} RAX: {} ORIG_RAX: {} RDI: {} RSI: {} RCX: {} RDX: {} RBX: {}".format(*list(map(hex, [regs.rip, regs.rsp, regs.rax, regs.orig_rax, regs.rdi, regs.rsi, regs.rcx, regs.rdx, regs.rbx]))))
            with open('/proc/%d/maps' % pid, 'rb') as f:
                print(f.read().decode())
            fd = os.open('/proc/%d/mem' % pid, os.O_RDONLY)
            os.lseek(fd, 0x1000060, os.SEEK_SET)
            code = readn(fd, 0x50)
            print(code)
            os.close(fd)
            break
            ...
            if parsed_status[3] == 'PTRACE_EVENT_SECCOMP':
                res = ptrace(PTRACE_GETREGS, pid, 0, ctypes.addressof(regs))
                print("RIP:{} RSP: {} RAX: {} ORIG_RAX: {} RDI: {} RSI: {} RDX: {} RCX: {}".format(*list(map(hex, [regs.rip, regs.rsp, regs.rax, regs.orig_rax, regs.rdi, regs.rsi, regs.rdx, regs.rcx]))))
                syscall_number = regs.orig_rax
                syscall_name = syscall_table[regs.orig_rax] if regs.orig_rax < len(syscall_table) else 'unk'
                print("Handling syscall {}!".format(syscall_name))
